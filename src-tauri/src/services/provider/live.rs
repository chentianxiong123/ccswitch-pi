...
pub fn import_pi_agent_providers_from_live(state: &AppState) -> Result<usize, AppError> {
    ...
    for (id, config) in providers {
        ...
        let settings_config = match serde_json::to_value(&config) {
            Ok(v) => v,
            Err(e) => {
                log::warn!("Failed to serialize Pi-Agent provider '{id}': {e}");
                continue;
            }
        };
                ...
                Ok(Some(existing)) => {
                    if existing.settings_config != settings_config {
                        ...
                        if let Err(e) = state.db.save_provider("pi-agent", &provider) {
                            ...
                            updated += 1;
                            log::info!("Updated Pi-Agent provider '{id}' from live config");
                        }
                    }
                }
                Ok(None) => {
                    log::warn!("Pi-Agent provider '{id}' disappeared while importing live config")
                }
                Err(e) => log::warn!("Failed to look up Pi-Agent provider '{id}': {e}"),
            }
            continue;
        }
